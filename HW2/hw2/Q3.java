...
public class Q3 {
	...
	static long model(int n) {
		...
		// Final: S(N) = log_2(N) + N + 1 
		return (long) ((Math.log(n)/Math.log(2)) + n + 1);
	}
	...
	static long bonusModel(int n) {
		return (long) ((0.1905*(n*n*n)) - (Math.floor(0.6667*(n*n))) + (0.6667*n) - (0.1904));
	}
	
	/** Launch the experiment. */
	public static void main(String[] args) {
		System.out.println("    N          Val      Actual   Predicted     Bonus");
		for (int n = 2; n <= 4096; n *= 2) {
			actual = 0;
			long predicted = model(n);
			long bonus = bonusModel(n);
			...
			System.out.println(String.format("%6d\t %12d %6d %12d %12d", n, val, actual, predicted, bonus));
		}
	}
}
